-- Keep multi-table writes together. These functions are SECURITY INVOKER so
-- the caller's RLS policies remain the authorization boundary.
...
begin
  ...
  from (
    select distinct unnest(coalesce(p_member_ids, '{}'::uuid[])) as member_id
  ) members
